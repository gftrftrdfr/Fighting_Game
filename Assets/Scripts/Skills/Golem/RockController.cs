    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if(canDame) 
        {
            if (tag == "Player 1")
            {
                ...
                {
                    collision.gameObject.GetComponent<CharacterMovement>().TakeDamage(150);
                    collision.gameObject.GetComponent<CharacterMovement>().Stun(1f, 0, collision.gameObject.GetComponent<Rigidbody2D>().mass * 100);
                    check = true;
                    canDame = false;
                }
            }
            ...
            else if (collision.gameObject.tag == "LimitMap")
            {
                check = true;
                canDame = false;
            }
